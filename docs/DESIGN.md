...
- ❌ Replacing Team Aegis — `snapit` *feeds* the Playwright/QA pipeline, it doesn't
  replace it.
...
- **Phases 1–2 (screenshot, video)** are *OS-level* — capture any pixel on screen.
- **Phases 3–4 (observe actions, fill fields)** are *browser-level* — only observable
...
## 8. Phase 3 — Live session analysis → integration test planning  *(R&D)*
...
- **Generate**: Playwright's own `codegen`/trace gets ~60% for free (record → code).
  The differentiated, genuinely hard part is *planning* meaningful tests: grouping
